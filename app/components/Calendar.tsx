...
const Calender: FC<PageProps> = ({ demo }) => {
    ...
    return (
        <div className="flex flex-col overflow-hidden gap-1 py-4 sm:py-6 md:py-10">
            {[...Array(numRows).keys()].map((row: number, rowIndex: number) => {
                return (
                    <div key={rowIndex} className="grid grid-cols-7 gap-1">
                        {dayList.map((dayOfWeek: string, dayOfWeekIndex: number) => {
                            let dayIndex = (row * 7) + dayOfWeekIndex - (firstDayOfMonth - 1)
                            ...
                            let color = demo ?
                                ...
                                dayIndex in demoData ? gradients.pink[demoData[dayIndex]] : 'white'

                            return (
                                <div style={{ background: color }} key={dayOfWeekIndex}
                                    className={`text-xs sm:text-sm border border-solid p-2 flex items-center gap-2 justify-between rounded-lg ${isToday ? 'border-pink-400' : 'border-pink-100'}
